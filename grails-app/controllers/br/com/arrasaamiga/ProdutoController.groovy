...
class ProdutoController {
    ...
    def quantidadeEmEstoque(Long produtoId, String unidade) {
        ...
        if (!produtoInstance) {
            ...
            log.info("Erro ao carregar produto com id ${produtoId} em ProdutoController#quantidadeEmEstoque")
            return
        }

        if (!unidade) {
            render status: NOT_FOUND
            log.info("Alguem tentanto carregar o produto ${produtoId} sem unidade ....")
            return
        }

        if (!produtoInstance.unidades.contains(unidade)) {
            render status: NOT_FOUND
            log.info("Não existe a unidade ${params.unidade} no produto ${produtoId} ....")
            return
        }
