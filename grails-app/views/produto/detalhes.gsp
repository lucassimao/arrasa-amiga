			...
			<h3 style="color:#01A8DD;"> ${produtoInstance.nome} </h3>
      		...
      		<div class="row-fluid" style="border:2px dotted pink;padding:10px;clear:both;">
      			...
      			<div class="span7" style="text-align:justify;position:relative;height:380px;">
					...
					<h3 style="color:#39c61f;position:absolute;bottom:60px;left:0px;">  R$ ${produtoInstance.precoAVistaEmReais} <i class="icon-tag"></i> </h3>
					...
					<g:form action="add" controller="shoppingCart" method="post" id="${produtoInstance.id}"> 
						...
						<g:if test="${estoque.quantidade > 0}">
							...
							<g:select style="position:absolute;bottom:10px;left:0px;width:60px;" 
								name="quantidade" from="${1..estoque.quantidade}"/>
						</g:if>
						<g:else>
							<span style="position:absolute;bottom:10px;left:0px;"> Estoque esgotado </span>
						</g:else>
